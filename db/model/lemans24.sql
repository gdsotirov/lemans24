...
CREATE TABLE IF NOT EXISTS `lemans24`.`drivers` (
  ...
  `full_name` VARCHAR(64) GENERATED ALWAYS AS (CONCAT(CASE WHEN title NOT LIKE 'Baron %' AND title NOT LIKE 'Earl %' AND title NOT LIKE 'Lord %' THEN CONCAT(title, ' ') ELSE '' END, CASE WHEN fname IS NULL THEN '' ELSE CONCAT(fname, ' ') END, CASE WHEN nickname IS NULL THEN '' ELSE CONCAT('"', nickname, '" ') END, lname, CASE WHEN title LIKE 'Baron %' OR title LIKE 'Earl %' OR title LIKE 'Lord %' THEN CONCAT(', ', title) ELSE '' END, CASE WHEN nm_suffix IS NULL THEN '' ELSE CASE WHEN nm_suffix LIKE 'Lord %' THEN CONCAT(', ', nm_suffix) ELSE CONCAT(' ', nm_suffix) END END)) VIRTUAL COMMENT 'Driver full name including title, names, nickname and suffix',
  `sex` ENUM('M', 'F') NULL,
  `born` DATE NULL,
...
-- -----------------------------------------------------
-- Placeholder table for view `lemans24`.`FemaleDriversByCountry`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `lemans24`.`FemaleDriversByCountry` (`id` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `lemans24`.`AllDriversResults` (`id` INT);
...
-- -----------------------------------------------------
-- View `lemans24`.`FemaleDriversByCountry`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `lemans24`.`FemaleDriversByCountry`;
USE `lemans24`;
